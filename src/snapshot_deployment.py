...
def print_all_snapshots():
    ...
    snapshot_1 = SnapshotService().set_resource(ec2_rescource_service)
...
def print_snapshots_filted_on_volume_size():
    ec2_client_service = EC2Client()
    snapshot_2 = SnapshotService().set_client(ec2_client_service)
           
    print("printing using client...")        
    volume_sizes = [8]
    for snapshot in snapshot_2.get_snapshots_filted_on_volume_size(volume_sizes, filters = []):
        print(snapshot)
    ...
    print("printing using resource...")
    volume_sizes = ["1"]
    for snapshot in snapshot_1.get_snapshots_filted_on_volume_size(volume_sizes, filters = []):
        print(snapshot)    
        
if __name__ == '__main__':
    print("Testing snapshot deployments")
    print_all_snapshots()
    print_snapshots_filted_on_volume_size()

...
def print_all_buckets():
    s3_resource = S3Resource(profile_name = "s3_developer").get_instance()
    s3 = S3().set_resource(s3_resource)
    
    print("Printing all buckets using AWS resource object...")
    for bucket in s3.get_all_buckets():
        print(bucket)

    s3_client = S3Client(profile_name = "s3_developer").get_client()
    s3_new = S3().set_client(s3_client)
